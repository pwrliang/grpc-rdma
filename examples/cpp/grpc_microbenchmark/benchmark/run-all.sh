set -e
export HELLOWORLD_HOME=/users/PAS0350/geng161/grpc-rdma/examples/cpp/grpc_microbenchmark/build
...
function set_hostfile() {
  ...
  hostfile="/tmp/grpc_mb_${USER}_${name_prefix}"
    ...
    for grpc_mode in "${GRPC_MODES[@]}"; do
      ...
      for j in "${!N_CLIENTS[@]}"; do
        ...
        ./run.sh --server_thread=$thread \
          ...
          --req="$REQ" \
          --resp="$RESP" \
          --batch=1000 --bp-timeout=$bp_to # --profiling=milli
      done
    done
  done
}
