...
class CommSpec {
 ...
 private:
  __attribute__((no_sanitize_address)) void initLocalInfo(
    ...
    int max_host_len = 128;
    char hn[max_host_len];
    int hn_len;
    ...
    if (hostname.empty()) {
      ...
      hn_len =
          hostname.length() > max_host_len ? max_host_len : hostname.length();
      strncpy(hn, hostname.c_str(), hn_len);
    }

    char* recv_buf =
        reinterpret_cast<char*>(calloc(worker_num_, sizeof(hn) * 10));
    MPI_Allgather(hn, max_host_len, MPI_CHAR, recv_buf, max_host_len, MPI_CHAR,
                  comm_);

    std::vector<std::string> worker_host_names(worker_num_);
    for (int i = 0; i < worker_num_; ++i) {
      worker_host_names[i].assign(&recv_buf[i * max_host_len],
                                  strlen(&recv_buf[i * max_host_len]));
    }
    free(recv_buf);
