...
struct Connections {
  ...
  void CreateBPConnection(int fd) {
    auto conn = std::make_shared<RDMASenderReceiverBP>(fd, true);
    int rank;
  ...
  void CreateEventConnection(int fd) {
    auto conn = std::make_shared<RDMASenderReceiverEvent>(fd, true);
    int rank;
  ...
  void CreateBPEVConnection(int fd) {
    auto conn = std::make_shared<RDMASenderReceiverBPEV>(fd, true);
    int rank;
  ...
  if (config->dir == Dir::kBi || config->dir == Dir::kC2S) {
    ...
    while (n_alive_conn > 0) {
      for (int i = 0; i < conns->rdma_conns.size(); i++) {
        ...
        if (rdmasr != nullptr && rdmasr->HasMessage()) {
          size_t rest_mlen = rdmasr->MarkMessageLength();
 ...
 public:
  ...
  void start(int port) {
    ...
    while (client_id < n_client) {
      ...
      gpr_log(GPR_INFO, "IP address: %s, client id: %d\n", s, client_id);
      int flag = 1;
