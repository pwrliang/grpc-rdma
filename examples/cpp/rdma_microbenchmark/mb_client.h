...
class RDMAClient {
 public:
  ...
  void connect(const char* server_address, int port) {
    ...
    do {
      ...
      if (r < 0) {
        ...
        gpr_log(GPR_ERROR,
                "Client %s rank %d cannot connect to server %s, errno: %d", hn,
                rank, server_address, errno);
        sleep(1);
      }
    } while (r < 0);
  ...
  void RunBusyPolling() {
    ...
    RDMASenderReceiverBP rdmasr(sockfd_, false);
    rdmasr.Init();
    set_should_work();
    ...
    auto send_msg = [&] {
      ...
      while (rdmasr.Send(&msghdr_out, &sz) != 0) {
      }
    };
    ...
    switch (config_.dir) {
      ...
      case Dir::kS2C: {
        ...
        while (running) {
          while (!rdmasr.HasMessage()) {
          }

          size_t mlens = rdmasr.MarkMessageLength();
          while (mlens > 0) {
            ssize_t sz;
            if (rdmasr.Recv(&msghdr_in, &sz) == 0) {
              mlens -= sz;
      ...
      case Dir::kBi: {
        while (n_iter < batch_size && config_.should_work) {
          ...
          size_t mlens = rdmasr.MarkMessageLength();
          ssize_t sz;
          GPR_ASSERT(rdmasr.Recv(&msghdr_in, &sz) == 0);
          mlens -= sz;
          GPR_ASSERT(mlens == 0);
  ...
  void RunEpoll() {
    RDMASenderReceiverEvent rdmasr(sockfd_, false);
    rdmasr.Init();
    set_should_work();
    ...
    auto send_msg = [&] {
      ...
      GPR_ASSERT(rdmasr.Send(&msghdr_out, &sz) == 0);
      rdmasr.SetMetadataReady();
      rdmasr.MarkMessageLength();
      //      failed_cnt++;
    };
    ...
    switch (config_.dir) {
      ...
      case Dir::kS2C: {
        ...
        auto process_event = [&](RDMASenderReceiverEvent* rdmasr) {
          auto rest_mlen = rdmasr->MarkMessageLength();
  ...
  void RunBPEV() {
    RDMASenderReceiverBPEV rdmasr(sockfd_, false);
    rdmasr.Init();
    set_should_work();
    // Waiting for all clients connected
    if (config_.mpi_server) {
