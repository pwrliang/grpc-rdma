...
class SyncServeice final : public BENCHMARK::Service {
 public:
  Status SayHello(ServerContext* context, const Data_Empty* request,
                  Data_Empty* reply) override {
    return Status::OK;
  }

  Status BiUnary(ServerContext* context, const Complex* request,
                 Complex* reply) override {
  ...
  Status ClientStream(ServerContext* context, ServerReader<Complex>* reader,
                      Complex* reply) override {
    Complex request;
  ...
  Status ServerStream(ServerContext* context, const Complex* request,
                      ServerWriter<Complex>* writer) override {
  ...
  Status BiStream(ServerContext* context,
                  ServerReaderWriter<Complex, Complex>* stream) override {
  ...
  ::grpc::Status Test(::grpc::ServerContext* context,
                      const ::benchmark::TestReq* request,
...
class BenchmarkServer {
 public:
  ...
  BenchmarkServer(const std::string server_address, bool sync_enable,
                  size_t sync_thread_num, bool async_enable,
                  size_t async_threads_num, size_t async_cqs_num)
      : sync_enable_(sync_enable),
        sync_threads_num_(sync_thread_num),
   ...
   public:
    ...
    void Proceed() {
      if (status_ == CREATE) {
        ...
        switch (type_) {
          case SAYHELLO:
            service_->RequestSayHello(&ctx_, &say_hello_pkg_->request,
                                      &say_hello_pkg_->responder, call_cq_,
                                      notification_cq_, this);
            break;
          case BIUNARY:
            service_->RequestBiUnary(&ctx_, &bi_unary_pkg_->request,
                                     &bi_unary_pkg_->responder, call_cq_,
                                     notification_cq_, this);
            break;
          case CLIENTSTREAM:
            ...
            break;
          case SERVERSTREAM:
            service_->RequestServerStream(&ctx_, &server_stream_pkg_->request,
                                          &server_stream_pkg_->responder,
                                          call_cq_, notification_cq_, this);
            break;
          case BISTREAM:
    ...
    void ProcessClientStream() {
      // ServerAsyncReader<Complex, Complex>& responder =
      // client_stream_pkg_->responder; Complex& request =
      // client_stream_pkg_->request; Complex& reply =
      // client_stream_pkg_->reply;
    ...
    void ProcessBiStream() {
      // ServerAsyncReaderWriter<Complex, Complex>& responder =
      // bi_stream_pkg_->responder; Complex& request = bi_stream_pkg_->request;
...
int main(int argc, char** argv) {
  ...
  BenchmarkServer server(server_address, sync_enable, sync_thread_num,
