...
function throughput() {
  ...
  for use_rdma in true false; do
    for n_clients in "${clients[@]}"; do
      ...
      export GRPC_ENABLE_RDMA_SUPPORT=$use_rdma
      export LOG_NAME="tput_rdma_${use_rdma}_cli_${n_clients}_concurrent_${concurrent}_streaming_${streaming}"
...
function varying_clients_latency() {
  ...
  for use_rdma in true false; do
    for n_clients in "${clients[@]}"; do
      ...
      export GRPC_ENABLE_RDMA_SUPPORT=$use_rdma
      export LOG_NAME="lat_rdma_${use_rdma}_cli_${n_clients}_concurrent_${concurrent}_streaming_${streaming}"
      ./run.sh --server-thread=$server_thread \
        --cqs=$cqs \
        --req=$req \
        --resp=$resp \
...
function varying_busy_polling_time() {
  ...
  for bp_timeout in "${time_list[@]}"; do
    export GRPC_ENABLE_RDMA_SUPPORT=true
    export LOG_NAME="tput_cli_${n_clients}_bp_timeout_${bp_timeout}"
    ./run.sh --server-thread=$server_thread \
      --cqs=$cqs \
      --req=$req \
      --resp=$resp \
