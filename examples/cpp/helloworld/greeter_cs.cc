...
int main(int argc, char** argv) {
 ...
 std::vector<std::string> hosts = split(FLAGS_host, ',');
 std::vector<std::thread> client_ths;
 int worker_id = FLAGS_worker_id;

 if (worker_id == -1) {
   gpr_log(GPR_INFO, "Invalid worker id %d", worker_id);
   abort();
 }
 ...
 for (int i = 0; i < hosts.size(); i++) {
   if (i != worker_id) {
     client_ths.emplace_back(
         [&](int i) {
           auto& host = hosts[i];
           gpr_log(GPR_INFO, "Server %d is connection to client %d, host: %s",
                   worker_id, i, host.c_str());
           GreeterClient greeter(grpc::CreateCustomChannel(
               host + ":50051", grpc::InsecureChannelCredentials(), args));

           int rpc_count = FLAGS_batch;
           std::vector<std::thread> ths;
           int server_worker_id = i;

           ths.emplace_back([rpc_count, server_worker_id, &greeter]() {
             CompletionQueue cq;
             ...
             gpr_log(GPR_INFO, "Worker %d Exit", server_worker_id);
           });
           ths.emplace_back([rpc_count, server_worker_id, &greeter]() {
             CompletionQueue cq;
             for (int i = 0; i < rpc_count; i++) {
             ...
             gpr_log(GPR_INFO, "Worker %d Exit1", server_worker_id);
           });
           ths.emplace_back([rpc_count, server_worker_id, &greeter]() {
             CompletionQueue cq;
             for (int i = 0; i < rpc_count; i++) {
             ...
             gpr_log(GPR_INFO, "Worker %d Exit2", server_worker_id);
           });
           ths.emplace_back([rpc_count, server_worker_id, &greeter]() {
             CompletionQueue cq;
             for (int i = 0; i < rpc_count; i++) {
             ...
             gpr_log(GPR_INFO, "Worker %d Exit3", server_worker_id);
           });
           for (auto& th : ths) {
             th.join();
           }
