...
class GPR_DLL ConfigVars {
 public:
  struct Overrides {
    ...
    std::optional<int32_t> rdma_port_num;
    std::optional<int32_t> rdma_gid_index;
    std::optional<int32_t> rdma_poller_thread_num;
    std::optional<int32_t> rdma_busy_polling_timeout_us;
    std::optional<int32_t> rdma_poller_sleep_timeout_ms;
    std::optional<int32_t> rdma_ring_buffer_size_kb;
    ...
    std::optional<bool> enable_rdma_support;
    ...
    std::optional<std::string> rdma_device_name;
  ...
  // A threshold hold to determine whether resort to epoll to wait for incoming
  // messages in microseconds
  int32_t RdmaBusyPollingTimeoutUs() const {
    return rdma_busy_polling_timeout_us_;
  }
  // Putting polling threads into sleep if no connections are found within the
  // timeout
 ...
 private:
  ...
  std::optional<std::string> override_rdma_device_name_;
};
