...
#define GRPC_ENABLE_RDMA_SUPPORT_DEFAULT true
#else
#define GRPC_ENABLE_RDMA_SUPPORT_DEFAULT false
#endif  // GRPC_USE_IBVERBS
...
ABSL_FLAG(std::optional<bool>, grpc_cpp_experimental_disable_reflection, {},
          ...
          "automatically added.");
ABSL_FLAG(std::optional<bool>, grpc_enable_rdma_support, {},
          "Enable RDMA support");
ABSL_FLAG(std::optional<int32_t>, grpc_rdma_port_num, {},
          "The port number of RDMA device being used");
ABSL_FLAG(std::optional<int32_t>, grpc_rdma_gid_index, {},
          "The GID index of RDMA device being used");
ABSL_FLAG(std::optional<int32_t>, grpc_rdma_poller_thread_num, {},
          "How many polling threads are used to detect incoming messages");
ABSL_FLAG(std::optional<int32_t>, grpc_rdma_busy_polling_timeout_us, {},
          "A threshold hold to determine whether resort to epoll to wait for "
          "incoming messages in microseconds");
ABSL_FLAG(std::optional<int32_t>, grpc_rdma_poller_sleep_timeout_ms, {},
          "Putting polling threads into sleep if no connections are found "
          "within the timeout");
ABSL_FLAG(std::optional<int32_t>, grpc_rdma_ring_buffer_size_kb, {},
          "Ring buffer size in KB per connection");
ABSL_FLAG(std::optional<std::string>, grpc_rdma_device_name, {},
          "RDMA device name");
...
ConfigVars::ConfigVars(const Overrides& overrides)
    : client_channel_backup_poll_interval_ms_(
      ...
      enable_rdma_support_(LoadConfig(
          FLAGS_grpc_enable_rdma_support, "GRPC_ENABLE_RDMA_SUPPORT",
          overrides.enable_rdma_support, GRPC_ENABLE_RDMA_SUPPORT_DEFAULT)),
...
std::string ConfigVars::ToString() const {
  return absl::StrCat(
      ...
      ", rdma_device_name: ", "\"", absl::CEscape(RdmaDeviceName()), "\"");
}
