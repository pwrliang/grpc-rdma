...
#ifdef GRPC_POSIX_SOCKET_TCP
class RdmaEngineListenerImpl
    : public std::enable_shared_from_this<RdmaEngineListenerImpl> {
 public:
  RdmaEngineListenerImpl(
 ...
 private:
  ...
  // RdmaEngineListenerImpl object. So the RdmaEngineListenerImpl can be
  ...
  class AsyncConnectionAcceptor {
   public:
    AsyncConnectionAcceptor(std::shared_ptr<EventEngine> engine,
                            std::shared_ptr<RdmaEngineListenerImpl> listener,
   ...
   private:
    ...
    std::shared_ptr<RdmaEngineListenerImpl> listener_;
  ...
  class ListenerAsyncAcceptors : public ListenerSocketsContainer {
   public:
    explicit ListenerAsyncAcceptors(RdmaEngineListenerImpl* listener)
        : listener_(listener){};
   ...
   private:
    ...
    RdmaEngineListenerImpl* listener_;
  };
...
class RdmaEngineListener : public PosixListenerWithFdSupport {
 public:
  RdmaEngineListener(
      ...
      std::unique_ptr<grpc_event_engine::experimental::MemoryAllocatorFactory>
          memory_allocator_factory,
      PosixEventPoller* poller, std::shared_ptr<EventEngine> engine)
      : impl_(std::make_shared<RdmaEngineListenerImpl>(
            std::move(on_accept), std::move(on_shutdown), config,
            std::move(memory_allocator_factory), poller, std::move(engine))) {}
  ~RdmaEngineListener() override { ShutdownListeningFds(); };
  absl::StatusOr<int> Bind(
 ...
 private:
  std::shared_ptr<RdmaEngineListenerImpl> impl_;
...
class RdmaEngineListener : public PosixListenerWithFdSupport {
 public:
  RdmaEngineListener() = default;
  ~RdmaEngineListener() override = default;
  ...
  absl::StatusOr<int> BindWithFd(
    ...
    grpc_core::Crash(
        "RdmaEngineListener::BindWithFd not supported on this "
        "platform");
  }
  absl::Status HandleExternalConnection(
      int /*listener_fd*/, int /*fd*/, SliceBuffer* /*pending_data*/) override {
    grpc_core::Crash(
        "RdmaEngineListener::HandleExternalConnection not "
        "supported on this platform");
  }
  void ShutdownListeningFds() override {
    grpc_core::Crash(
        "RdmaEngineListener::ShutdownListeningFds not supported on "
        "this platform");
  }
};
