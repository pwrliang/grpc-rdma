...
class RdmaAsyncConnect {
 ...
 private:
  ...
  int refs_{2};
  grpc_event_engine::experimental::RdmaEventHandle* fd_;
...
class RdmaEnginePollerManager
 ...
 public:
  ...
  ~RdmaEnginePollerManager() override;
...
class RdmaEventEngine final : public PosixEventEngineWithFdSupport,
 ...
 public:
  ...
  static std::shared_ptr<RdmaEventEngine> MakeTestOnlyPosixEventEngine(
      std::shared_ptr<grpc_event_engine::experimental::PosixEventPoller>
          test_only_poller) {
    return std::make_shared<RdmaEventEngine>(std::move(test_only_poller));
  }
#endif  // GRPC_POSIX_SOCKET_TCP
...
#ifdef GRPC_POSIX_SOCKET_TCP
  friend class RdmaAsyncConnect;
  struct ConnectionShard {
    grpc_core::Mutex mu;
    absl::flat_hash_map<int64_t, RdmaAsyncConnect*> pending_connections
        ABSL_GUARDED_BY(&mu);
  };
...
#ifdef GRPC_POSIX_SOCKET_TCP
  std::shared_ptr<RdmaEnginePollerManager> poller_manager_;
#endif  // GRPC_POSIX_SOCKET_TCP
};
...
#endif  // GRPC_SRC_CORE_LIB_EVENT_ENGINE_RDMA_ENGINE_RDMA_ENGINE_H
