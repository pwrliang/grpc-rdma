...
RdmaEngineListenerImpl::RdmaEngineListenerImpl(
...
absl::StatusOr<int> RdmaEngineListenerImpl::Bind(
...
void RdmaEngineListenerImpl::AsyncConnectionAcceptor::Start() {
  Ref();
  handle_->NotifyOnRead(notify_on_accept_);
}

void RdmaEngineListenerImpl::AsyncConnectionAcceptor::NotifyOnAccept(
    absl::Status status) {
  ...
  for (;;) {
    ...
    auto* handle =
        dynamic_cast<RdmaEventHandle*>(listener_->poller_->CreateHandle(
    ...
    auto endpoint = CreateRdmaEndpoint(
        /*handle=*/handle,
...
absl::Status RdmaEngineListenerImpl::HandleExternalConnection(
  ...
  grpc_core::EnsureRunInExecCtx([this, peer_name = std::move(*peer_name),
    ...
    auto* handle = dynamic_cast<RdmaEventHandle*>(
        poller_->CreateHandle(fd, peer_name, poller_->CanTrackErrors()));
    absl::Status status;
    handle->InitializePair(status);
...
void RdmaEngineListenerImpl::AsyncConnectionAcceptor::Shutdown() {
...
absl::Status RdmaEngineListenerImpl::Start() {
  grpc_core::MutexLock lock(&this->mu_);
...
void RdmaEngineListenerImpl::TriggerShutdown() {
...
RdmaEngineListenerImpl::~RdmaEngineListenerImpl() {
  ...
  // shared_ptr ref to the parent RdmaEngineListenerImpl.
