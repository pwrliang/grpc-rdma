...
#include "src/core/lib/event_engine/rdma_engine/rdma_engine_listener.h"
#include "src/core/lib/event_engine/rdma_engine/rdma_poller_default.h"
...
void RdmaAsyncConnect::Start(EventEngine::Duration timeout) {
...
RdmaAsyncConnect ::~RdmaAsyncConnect() { delete on_writable_; }

void RdmaAsyncConnect::OnTimeoutExpired(absl::Status status) {
  bool done = false;
  {
...
void RdmaAsyncConnect::OnWritable(absl::Status status)
  ...
  auto on_writable_finish = absl::MakeCleanup([&]() -> void {
    mu_.AssertHeld();
    if (!connect_cancelled) {
      reinterpret_cast<RdmaEventEngine*>(engine_.get())
  ...
  switch (so_error) {
    case 0: {
      fd->InitializePair(status);
      if (!status.ok()) {
        return;
      }
      ep = CreateRdmaEndpoint(fd, nullptr, engine_, std::move(allocator_),
                              options_);
      fd = nullptr;
      break;
    }
...
EventEngine::ConnectionHandle RdmaEventEngine::ConnectInternal(
  ...
  if (connect_errno == 0) {
    ...
    if (!status.ok()) {
      ...
      Run([on_connect = std::move(on_connect), ep = status]() mutable {
        on_connect(std::move(ep));
      });
      return EventEngine::ConnectionHandle::kInvalid;
    }
    ...
    // attempts.
    Run([on_connect = std::move(on_connect),
         ep = CreateRdmaEndpoint(handle, nullptr, shared_from_this(),
  ...
  RdmaAsyncConnect* ac = new RdmaAsyncConnect(
...
void RdmaEventEngine::OnConnectFinishInternal(int connection_handle) {
...
RdmaEnginePollerManager::RdmaEnginePollerManager(
    std::shared_ptr<ThreadPool> executor)
    : poller_(grpc_event_engine::experimental::MakeDefaultRdmaPoller(this)),
...
void RdmaEnginePollerManager::Run(experimental::EventEngine::Closure* closure) {
  if (executor_ != nullptr) {
    executor_->Run(closure);
  }
}

void RdmaEnginePollerManager::Run(absl::AnyInvocable<void()> cb) {
  if (executor_ != nullptr) {
    executor_->Run(std::move(cb));
  }
}

void RdmaEnginePollerManager::TriggerShutdown() {
...
RdmaEventEngine::RdmaEventEngine(std::shared_ptr<PosixEventPoller> poller)
  ...
  poller_manager_ = std::make_shared<RdmaEnginePollerManager>(poller);
#endif
}

RdmaEventEngine::RdmaEventEngine()
    : connection_shards_(std::max(2 * gpr_cpu_num_cores(), 1u)),
      executor_(MakeThreadPool(grpc_core::Clamp(gpr_cpu_num_cores(), 4u, 16u))),
      timer_manager_(std::make_shared<TimerManager>(executor_)) {
  g_timer_fork_manager->RegisterForkable(
      ...
      TimerForkCallbackMethods::PostforkChild);
#if GRPC_PLATFORM_SUPPORTS_POSIX_POLLING
  poller_manager_ = std::make_shared<RdmaEnginePollerManager>(executor_);
...
void RdmaEventEngine::PollerWorkInternal(
...
struct RdmaEventEngine::ClosureData final : public EventEngine::Closure {
  absl::AnyInvocable<void()> cb;
  Timer timer;
  RdmaEventEngine* engine;
  EventEngine::TaskHandle handle;

  void Run() override {
    GRPC_TRACE_LOG(event_engine, INFO)
        << "RdmaEventEngine:" << engine << " executing callback:" << handle;
    {
...
RdmaEventEngine::~RdmaEventEngine() {
  {
    ...
    if (GRPC_TRACE_FLAG_ENABLED(event_engine)) {
      for (auto handle : known_handles_) {
        LOG(ERROR) << "(event_engine) RdmaEventEngine:" << this
...
bool RdmaEventEngine::Cancel(EventEngine::TaskHandle handle) {
  grpc_core::MutexLock lock(&mu_);
...
EventEngine::TaskHandle RdmaEventEngine::RunAfter(
...
void RdmaEventEngine::Run(absl::AnyInvocable<void()> closure) {
  executor_->Run(std::move(closure));
}

void RdmaEventEngine::Run(EventEngine::Closure* closure) {
  executor_->Run(closure);
}

EventEngine::TaskHandle RdmaEventEngine::RunAfterInternal(
  ...
  cd->handle = handle;
  GRPC_TRACE_LOG(event_engine, INFO)
      << "RdmaEventEngine:" << this << " scheduling callback:" << handle;
  timer_manager_->TimerInit(&cd->timer, when_ts, cd);
  return handle;
}

RdmaEventEngine::PosixDNSResolver::PosixDNSResolver(
    grpc_core::OrphanablePtr<RefCountedDNSResolverInterface> dns_resolver)
    : dns_resolver_(std::move(dns_resolver)) {}

void RdmaEventEngine::PosixDNSResolver::LookupHostname(
...
void RdmaEventEngine::PosixDNSResolver::LookupSRV(LookupSRVCallback on_resolve,
                                                  absl::string_view name) {
  dns_resolver_->LookupSRV(std::move(on_resolve), name);
}

void RdmaEventEngine::PosixDNSResolver::LookupTXT(LookupTXTCallback on_resolve,
                                                  absl::string_view name) {
  dns_resolver_->LookupTXT(std::move(on_resolve), name);
}

absl::StatusOr<std::unique_ptr<EventEngine::DNSResolver>>
RdmaEventEngine::GetDNSResolver(
  ...
  if (ShouldUseAresDnsResolver()) {
    ...
    GRPC_TRACE_LOG(event_engine_dns, INFO)
        << "RdmaEventEngine::" << this << " creating AresResolver";
    ...
    return std::make_unique<RdmaEventEngine::PosixDNSResolver>(
        std::move(*ares_resolver));
#endif  // GRPC_ARES == 1 && defined(GRPC_POSIX_SOCKET_ARES_EV_DRIVER)
  }
  GRPC_TRACE_LOG(event_engine_dns, INFO)
      << "RdmaEventEngine::" << this << " creating NativePosixDNSResolver";
  return std::make_unique<NativePosixDNSResolver>(shared_from_this());
#endif  // GRPC_POSIX_SOCKET_RESOLVE_ADDRESS
}

bool RdmaEventEngine::IsWorkerThread() { grpc_core::Crash("unimplemented"); }

bool RdmaEventEngine::CancelConnect(EventEngine::ConnectionHandle handle) {
#if GRPC_PLATFORM_SUPPORTS_POSIX_POLLING
...
EventEngine::ConnectionHandle RdmaEventEngine::Connect(
...
RdmaEventEngine::CreatePosixEndpointFromFd(int fd, const EndpointConfig& config,
  ...
  RdmaEventHandle* handle = dynamic_cast<RdmaEventHandle*>(
      poller->CreateHandle(fd, "rdma-server", poller->CanTrackErrors()));
  absl::Status status;

  handle->InitializePair(status);

  if (!status.ok()) {
    LOG(ERROR) << "Cannot initialize pair " << status.message();
  }

  return CreateRdmaEndpoint(handle, nullptr, shared_from_this(),
                            std::move(memory_allocator),
                            TcpOptionsFromEndpointConfig(config));
#else   // GRPC_PLATFORM_SUPPORTS_POSIX_POLLING
  grpc_core::Crash(
      "RdmaEventEngine::CreatePosixEndpointFromFd is not supported on "
      "this platform");
#endif  // GRPC_PLATFORM_SUPPORTS_POSIX_POLLING
}

std::unique_ptr<EventEngine::Endpoint> RdmaEventEngine::CreateEndpointFromFd(
...
RdmaEventEngine::CreateListener(
  ...
  return std::make_unique<RdmaEngineListener>(
...
absl::StatusOr<std::unique_ptr<EventEngine::Listener>>
RdmaEventEngine::CreatePosixListener(
