...
#ifndef GRPC_SRC_CORE_LIB_EVENT_ENGINE_RDMA_ENGINE_EV_BPEV_RDMA_LINUX_H
#define GRPC_SRC_CORE_LIB_EVENT_ENGINE_RDMA_ENGINE_EV_BPEV_RDMA_LINUX_H
#include <list>
#include <memory>
#include <string>
...
class BpevEventHandle;

// Definition of epoll1 based poller.
class BpevPoller : public grpc_event_engine::experimental::PosixEventPoller {
 public:
  explicit BpevPoller(Scheduler* scheduler);
 ...
 private:
  ...
  using Events = absl::InlinedVector<BpevEventHandle*, 5>;
  ...
  class HandlesList {
   public:
    explicit HandlesList(BpevEventHandle* handle) : handle(handle) {}
    BpevEventHandle* handle;
    BpevEventHandle* next = nullptr;
    BpevEventHandle* prev = nullptr;
  };
  friend class BpevEventHandle;
#ifdef GRPC_LINUX_EPOLL
  struct EpollSet {
    int epfd = -1;
...
#endif  // GRPC_SRC_CORE_LIB_EVENT_ENGINE_RDMA_ENGINE_EV_BPEV_RDMA_LINUX_H
