...
static bool epoll_set_init() {
  ...
  g_epoll_set.polling_timeout_us =
      grpc_core::ibverbs::Config::Get().get_busy_polling_timeout_us();
  return true;
}
...
static grpc_fd* fd_create(int fd, const char* name, bool track_err) {
  ...
  // TODO(liang): Why not create RDMASR here?
...
static void fd_shutdown_internal(grpc_fd* fd, grpc_error_handle why,
  ...
  if (fd->read_closure->SetShutdown(GRPC_ERROR_REF(why))) {
    if (!releasing_fd) {
      ...
      if (fd->pair != nullptr) {
        if (epoll_ctl(g_epoll_set.epfd, EPOLL_CTL_DEL,
                      fd->pair->get_wakeup_fd()->read_fd, &phony_event) != 0) {
          gpr_log(GPR_ERROR, "epoll_ctl failed: %s", strerror(errno));
        }
      }
    }
...
static void fd_set_arg(grpc_fd* fd, void* arg) {
  ...
  struct epoll_event ev;
  ev.events = static_cast<uint32_t>(EPOLLIN | EPOLLEXCLUSIVE | EPOLLET);
  /* Use the least significant bit of ev.data.ptr to represent the fd has rdmasr
   * object. We expect the addresses to be word aligned. */
  GPR_ASSERT((reinterpret_cast<intptr_t>(fd) & 2) == 0);
  ev.data.ptr = reinterpret_cast<void*>(reinterpret_cast<intptr_t>(fd) | 2);
  if (epoll_ctl(g_epoll_set.epfd, EPOLL_CTL_ADD, wakeup_fd, &ev) != 0) {
    gpr_log(GPR_ERROR, "epoll_ctl failed: %s", strerror(errno));
  }
}
/*******************************************************************************
...
static void reset_event_manager_on_fork() {
  ...
  gpr_mu_unlock(&fork_fd_list_mu);
  shutdown_engine();
  grpc_init_epoll1_rdma_linux(true);
}

/* It is possible that GLIBC has epoll but the underlying kernel doesn't.
 * Create epoll_fd (epoll_set_init() takes care of that) to make sure epoll
 * support is available */
const grpc_event_engine_vtable* grpc_init_epoll1_rdma_linux(
...
#include "src/core/lib/iomgr/ev_epoll1_rdma_bp_linux.h"
