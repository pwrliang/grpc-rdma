...
struct grpc_fd {
  grpc_fd(int fd, const char* name, bool track_err, int) : fd(fd) {
...
static grpc_error_handle pollable_process_events(grpc_pollset* pollset,
  ...
  for (int i = 0; (drain || i < handle_count) &&
    ...
    int ev_src = reinterpret_cast<intptr_t>(data_ptr) & 3;
