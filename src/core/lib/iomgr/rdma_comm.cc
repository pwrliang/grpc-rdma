...
#define GRPC_RDMA_COMM  // FIXME: Enabled by cmake

#ifdef GRPC_RDMA_COMM
...
RDMASenderReceiver* grpc_rdma_get_rdmasr(grpc_endpoint* ep) {
  grpc_rdma* rdma = reinterpret_cast<grpc_rdma*>(ep);
  GPR_ASSERT(ep->vtable == &vtable);
  return rdma->rdmasr;
}

void* grpc_rdma_require_zerocopy_sendspace(grpc_endpoint* ep, size_t size) {
  grpc_rdma* rdma = reinterpret_cast<grpc_rdma*>(ep);
  GPR_ASSERT(ep->vtable == &vtable);
  return rdma->rdmasr->RequireZerocopySendSpace(size);
}

#endif /* GRPC_RDMA_COMM */
