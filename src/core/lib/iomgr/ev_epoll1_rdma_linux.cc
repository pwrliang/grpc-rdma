...
static void fd_orphan(grpc_fd* fd, grpc_closure* on_done, int* release_fd,
  ...
  if (fd->pair != nullptr) {
    ...
    epoll_ctl(g_epoll_set.epfd, EPOLL_CTL_DEL, wakeup_fd, &ev_fd);
    fd->pair = nullptr;
  }
...
static void fd_set_arg(grpc_fd* fd, void* arg) {
  ...
  if (epoll_ctl(g_epoll_set.epfd, EPOLL_CTL_ADD, wakeup_fd, &wakeup_ep_ev) !=
      0) {
    switch (errno) {
      case EEXIST:
        break;
...
static grpc_error_handle process_epoll_events(grpc_pollset* /*pollset*/) {
  ...
  for (int idx = 0;
    ...
    if (data_ptr == &global_wakeup_fd) {
      ...
      if (reinterpret_cast<intptr_t>(data_ptr) & 2) {
...
const grpc_event_engine_vtable grpc_ev_epoll1_rdma_posix = {
    ...
    [](bool) {
      ...
      return config.EnableRdmaSupport() && init_epoll1_linux();
    },
    /* init_engine = */
    []() { CHECK(init_epoll1_linux()); }, shutdown_background_closure,
    /* shutdown_engine = */
