...
// Why epollex is removed https://github.com/grpc/grpc/pull/29160
// performance issue: https://github.com/grpc/grpc/issues/34645
#define MAX_EPOLL_EVENTS 100
#define MAX_FDS_IN_CACHE 32

grpc_core::DebugOnlyTraceFlag grpc_trace_pollable_refcount_bpev(
...
struct grpc_fd {
  grpc_fd(int fd, const char* name, int) : fd(fd) {
    ...
    GRPC_TRACE_LOG(fd_refcount, INFO)
        << "FD " << fd << " grpc fd " << this << " create " << fd_name;
  }
...
static void pollable_unref(pollable* p, const grpc_core::DebugLocation& dbg_loc,
  ...
  if (GPR_UNLIKELY(p != nullptr && p->refs.Unref(dbg_loc, reason))) {
    GRPC_TRACE_LOG(fd_trace, INFO)
        << "pollable_unref: Closing epfd: " << p->epfd;
    close(p->epfd);
    ...
    auto fds = *p->rdma_fds;  // need a copy
    gpr_mu_unlock(&p->rdma_mu);

    for (auto fd : fds) {
      gpr_mu_lock(&fd->rdma_mu);
      fd->polling_by->erase(
          std::remove(fd->polling_by->begin(), fd->polling_by->end(), p),
          fd->polling_by->end());
...
static grpc_fd* fd_create(int fd, const char* name, bool track_err) {
  ...
  return new (new_fd) grpc_fd(fd, name, 0);
}
...
static void fd_orphan(grpc_fd* fd, grpc_closure* on_done, int* release_fd,
  ...
  if (!is_fd_closed) {
    GRPC_TRACE_LOG(fd_trace, INFO) << "epoll_fd " << fd << "(" << fd->fd
                                   << ") was orphaned but not closed.";
  }
...
static void fd_shutdown(grpc_fd* fd, grpc_error_handle why) {
  if (fd->read_closure.SetShutdown(why)) {
    if (shutdown(fd->fd, SHUT_RDWR)) {
      if (errno != ENOTCONN) {
        LOG(ERROR) << "Error shutting down fd " << grpc_fd_wrapped_fd(fd)
                   << ". errno: " << errno;
      }
    }
...
static grpc_error_handle pollable_create(pollable_type type, pollable** p) {
  *p = nullptr;
  ...
  GRPC_TRACE_LOG(fd_trace, INFO)
      << "Pollable_create: created epfd: " << epfd << " (type: " << type << ")";
  *p = static_cast<pollable*>(gpr_malloc(sizeof(**p)));
  grpc_error_handle err = grpc_wakeup_fd_init(&(*p)->wakeup);
  if (!err.ok()) {
    GRPC_TRACE_LOG(fd_trace, ERROR)
        << "Pollable_create: closed epfd: " << epfd << " (type: " << type
        << "). wakeupfd_init error";
    close(epfd);
    gpr_free(*p);
    *p = nullptr;
  ...
  (*p)->polling_timeout_us = config.RdmaBusyPollingTimeoutUs();
...
static grpc_error_handle pollable_add_fd(pollable* p, grpc_fd* fd) {
  ...
  if (fd->pair != nullptr) {
    ...
    if (p->root_worker != nullptr) {  // Kick a worker so pollable epoll can
      ...
      append_error(&error, kick_one_worker(p->root_worker), err_desc);
    }
  }
...
static void pollset_maybe_finish_shutdown(grpc_pollset* pollset) {
  ...
  //            " "rw=%p (target:NULL) cpsc=%d (target:0)", pollset,
  //            pollset->active_pollable, pollset->shutdown_closure,
...
static grpc_error_handle kick_one_worker(grpc_pollset_worker* specific_worker) {
  ...
  CHECK(specific_worker != nullptr);
  if (specific_worker->kicked) {
    GRPC_TRACE_LOG(polling, INFO)
        << "PS:" << p << " kicked_specific_but_already_kicked";
    return absl::OkStatus();
  }
  if (g_current_thread_worker == specific_worker) {
    GRPC_TRACE_LOG(polling, INFO) << "PS:" << p << " kicked_specific_but_awake";
    specific_worker->kicked = true;
    return absl::OkStatus();
  }
  if (specific_worker == p->root_worker) {
    GRPC_TRACE_LOG(polling, INFO)
        << "PS:" << p << " kicked_specific_via_wakeup_fd";
    specific_worker->kicked = true;
    grpc_error_handle error = grpc_wakeup_fd_wakeup(&p->wakeup);
    return error;
  }
  if (specific_worker->initialized_cv) {
    GRPC_TRACE_LOG(polling, INFO) << "PS:" << p << " kicked_specific_via_cv";
    specific_worker->kicked = true;
...
static grpc_error_handle pollset_kick(grpc_pollset* pollset,
  ...
  if (specific_worker == nullptr) {
    if (g_current_thread_pollset != pollset) {
      ...
      GRPC_TRACE_LOG(polling, INFO)
          << "PS:" << pollset << " kicked_any_but_awake";
      return absl::OkStatus();
    }
  } else {
...
static void pollset_shutdown(grpc_pollset* pollset, grpc_closure* closure) {
  CHECK(pollset->shutdown_closure == nullptr);
...
static grpc_error_handle pollable_process_events(grpc_pollset* pollset,
  ...
  for (int i = 0; (drain || i < handle_count) &&
    ...
    if (1 & reinterpret_cast<intptr_t>(data_ptr)) {
      GRPC_TRACE_LOG(polling, INFO)
          << "PS:" << pollset << " got pollset_wakeup " << data_ptr;
      append_error(
          &error,
      ...
      GRPC_TRACE_LOG(polling, INFO)
          << "PS:" << pollset << " got fd " << fd << ": cancel=" << cancel
          << " read= " << read_ev << "write=" << write_ev;
      if (read_ev || cancel) {
        fd_become_readable(fd);
      }
      if (write_ev || cancel) {
...
static grpc_error_handle pollable_epoll(pollable* p,
  ...
  GRPC_TRACE_LOG(polling, INFO) << "POLLABLE:" << p << "[" << pollable_desc(p)
                                << "] poll for " << timeout << "ms";
  if (timeout != 0) {
  ...
  if (timeout >= 0) {
    polling_timeout_us = std::min(timeout * 1000, polling_timeout_us);
  }
...
static bool begin_worker(grpc_pollset* pollset, grpc_pollset_worker* worker,
  ...
  if (!worker_insert(&worker->pollable_obj->root_worker, worker,
    ...
    if (worker->pollable_obj->root_worker != worker) {
      GRPC_TRACE_LOG(polling, INFO)
          << "PS:" << pollset << " wait " << worker->pollable_obj
          << " w=" << worker << " for "
          << poll_deadline_to_millis_timeout(deadline) << "ms";
    }
    ...
    while (do_poll && worker->pollable_obj->root_worker != worker) {
      if (gpr_cv_wait(&worker->cv, &worker->pollable_obj->mu,
        ...
        GRPC_TRACE_LOG(polling, INFO)
            << "PS:" << pollset << " timeout_wait " << worker->pollable_obj
            << " w=" << worker;
        do_poll = false;
      } else if (worker->kicked) {
        GRPC_TRACE_LOG(polling, INFO)
            << "PS:" << pollset << " wakeup " << worker->pollable_obj
            << " w=" << worker;
        do_poll = false;
      } else if (worker->pollable_obj->root_worker != worker) {
        GRPC_TRACE_LOG(polling, INFO)
            << "PS:" << pollset << " spurious_wakeup " << worker->pollable_obj
            << " w=" << worker;
      }
    }
  ...
  switch (worker_remove(&worker->pollable_obj->root_worker, worker,
    ...
    case WRR_EMPTIED:
      if (pollset->active_pollable != worker->pollable_obj) {
        ...
        if (!pollable_process_events(pollset, worker->pollable_obj, true)
                 .ok()) {
          LOG(ERROR) << "PS " << pollset << " pollable " << worker->pollable_obj
                     << " pollable_process_events error";
        }
      }
      break;
    case WRR_REMOVED:
...
#endif
  //  GRPC_TRACE_LOG(polling, INFO) << absl::StrFormat(
  //      "PS:%p work hdl=%p worker=%p now=%ld deadline=%ld kwp=%d pollable=%p",
...
static grpc_error_handle pollset_transition_pollable_from_empty_to_fd_locked(
  ...
  grpc_error_handle error = absl::OkStatus();
  GRPC_TRACE_LOG(polling, INFO)
      << "PS:" << pollset << " add fd " << fd << " (" << fd->fd
      << "); transition pollable from empty to fd";
...
static grpc_error_handle pollset_as_multipollable_locked(
  ...
  switch (pollset->active_pollable->type) {
    case PO_EMPTY:
      ...
      GRPC_TRACE_LOG(polling, INFO)
          << "PS:" << pollset
          << " active pollable transition from empty to multi";
...
static void pollset_set_add_fd(grpc_pollset_set* pss, grpc_fd* fd) {
  ...
  if (pss->fd_count == pss->fd_capacity) {
    pss->fd_capacity = std::max(pss->fd_capacity * 2, 8ul);
...
static void pollset_set_del_fd(grpc_pollset_set* pss, grpc_fd* fd) {
  GRPC_TRACE_LOG(polling, INFO) << "PSS:" << pss << ": del fd " << fd;
...
static void pollset_set_del_pollset(grpc_pollset_set* pss, grpc_pollset* ps) {
  GRPC_TRACE_LOG(polling, INFO) << "PSS:" << pss << ": del pollset " << ps;
  pss = pss_lock_adam(pss);
  size_t i;
...
static void pollset_set_add_pollset(grpc_pollset_set* pss, grpc_pollset* ps) {
  GRPC_TRACE_LOG(polling, INFO) << "PSS:" << pss << ": add pollset " << ps;
  grpc_error_handle error = absl::OkStatus();
  ...
  if (pss->pollset_count == pss->pollset_capacity) {
    pss->pollset_capacity = std::max(pss->pollset_capacity * 2, 8ul);
...
static void pollset_set_add_pollset_set(grpc_pollset_set* a,
                                        grpc_pollset_set* b) {
  GRPC_TRACE_LOG(polling, INFO) << "PSS: merge (" << a << ", " << b << ")";
  grpc_error_handle error = absl::OkStatus();
  static const char* err_desc = "pollset_set_add_fd";
  for (;;) {
    if (a == b) {
  ...
  GRPC_TRACE_LOG(polling, INFO) << "PSS: parent " << b << " to " << a;
  a->refs.Ref();
  b->parent = a;
...
static void shutdown_engine(void) {
  grpc_core::ibverbs::Poller::Get().Shutdown();
  fd_global_shutdown();
  pollset_global_shutdown();
}

static bool init_epollex_linux();

const grpc_event_engine_vtable grpc_ev_epollex_rdma_posix = {
    sizeof(grpc_pollset), true, false,

    fd_create, fd_wrapped_fd, fd_orphan, fd_shutdown, fd_notify_on_read,
    fd_notify_on_write, fd_notify_on_error, fd_become_readable,
    fd_become_writable, fd_has_errors, fd_is_shutdown,

    pollset_init, pollset_shutdown, pollset_destroy, pollset_work, pollset_kick,
    pollset_add_fd,
    ...
    [](bool) {
      const grpc_core::ConfigVars& config = grpc_core::ConfigVars::Get();
      return config.EnableRdmaSupport() && init_epollex_linux();
    },
    /* init_engine = */
    []() { CHECK(init_epollex_linux()); }, shutdown_background_closure,
    /* shutdown_engine = */
    []() { shutdown_engine(); }, add_closure_to_background_poller,

    fd_set_pre_allocated, fd_set_arg};

static bool init_epollex_linux() {
  if (!grpc_has_wakeup_fd()) {
    LOG(ERROR) << "Skipping epollex because of no wakeup fd.";
    return false;
  }
