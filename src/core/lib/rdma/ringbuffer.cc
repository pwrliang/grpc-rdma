...
size_t RingBufferBP::check_mlen(size_t head) {
  if (head >= capacity_) {
    rdma_log(RDMA_ERROR, "RingBufferBP::check_mlen, head %d is out of range %d", head, capacity_);
    exit(-1);
  }
  ...
  if(head + sizeof(size_t) <= capacity_) {
    ...
    if (mlen && mlen + sizeof(size_t) + 1 < capacity_ && check_tail(head, mlen) != 1) {
      return 0;
    }
...
size_t RingBufferBP::read_to_msghdr(msghdr* msg, size_t head, size_t expected_read_size) {
  if (expected_read_size == 0) {
    rdma_log(RDMA_WARNING, "RingBufferBP::read_to_msghdr, expected read size == 0");
    return 0;
  }
  if (head >= capacity_) {
  ...
  size_t mlens = 0, read_size = 0, buf_offset = (head_ + sizeof(size_t)) % capacity_, n;
  while (read_size < expected_read_size && iov_idx < msg->msg_iovlen && mlen > 0) {
    iov_rlen = msg->msg_iov[iov_idx].iov_len - iov_offset; // rest space of current slice
    m_rlen = mlen - m_offset; // uncopied bytes for currecnt message (length of mlen)
    ...
    rdma_log(RDMA_INFO, "RingBufferBP::read_to_msghdr, read %d bytes from head %d",
             n, buf_offset);
    buf_offset += n;
    read_size += n;
    iov_offset += n;
    ...
    if (n == m_rlen) { 
      ...
      mlens += mlen;
      head = (head + sizeof(size_t) + mlen + 1) % capacity_; // move to next head
      mlen = check_mlen(head); // check mlen of the new head
      m_offset = 0; 
      read_size += 1 + sizeof(size_t); // add head and tail size to read_size

      // move buf_offset to the first place right after the head of the next message
      // if no next message, the loop will finish.
  ...
  if (read_size != expected_read_size) {
    rdma_log(RDMA_ERROR, "RingBufferBP::read_to_msghdr, read size (%d) != expected read size (%d)",
