...
//   int flags = fcntl(send_channel_->fd, F_GETFL);
//   if (fcntl(send_channel_->fd, F_SETFL, flags | O_NONBLOCK) < 0) {
...
//   int flags = fcntl(recv_channel_->fd, F_GETFL);
//   if (fcntl(recv_channel_->fd, F_SETFL, flags | O_NONBLOCK) < 0) {
//     rdma_log(RDMA_ERROR, "RDMAConnEvent::RDMAConnEvent, failed to change channel fd to non-blocking");
//     exit(-1);
//   }
...
//       ibv_ack_cq_events(rcq_, unacked_recv_events_num_);
//     }
//     ibv_destroy_comp_channel(recv_channel_);
//   }
//   if (send_channel_) {
//     if (unacked_send_events_num_) {
//       ibv_ack_cq_events(rcq_, unacked_send_events_num_);
//     }
...
//     if (ibv_post_send(qp_, &send_wrs_[sr_tail_], &bad_wr) != 0) {
