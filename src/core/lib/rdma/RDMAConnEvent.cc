...
RDMAConnEvent::RDMAConnEvent(int fd, RDMANode* node,
  ...
  rcq_ = std::shared_ptr<ibv_cq>(
      ibv_create_cq(ctx, DEFAULT_CQE, rdmasr, channel_, 0),
      [](ibv_cq* p) { ibv_destroy_cq(p); });
  if (!rcq_) {
    rdma_log(
        RDMA_ERROR,
...
RDMAConnEvent::~RDMAConnEvent() {
  if (channel_) {
    if (unacked_events_num_) {
      ibv_ack_cq_events(rcq_.get(), unacked_events_num_);
    }
...
bool RDMAConnEvent::get_event_locked() {
  ...
  if (cq != rcq_.get()) {
    rdma_log(RDMA_ERROR,
             "RDMAConnEvent::get_event_locked, unknown CQ got event");
    exit(-1);
  }
  unacked_events_num_++;
...
size_t RDMAConnEvent::get_events_locked(uint8_t* addr, size_t length,
                                        uint32_t lkey) {
  ibv_cq* cq = nullptr;
  void* ev_ctx = nullptr;
  ...
  // if (cq != rcq_) {
  //   rdma_log(RDMA_ERROR, "RDMAConnEvent::get_event_locked, unknown CQ got
  //   event"); exit(-1);
  // }
  // unacked_events_num_++;
  // if (ibv_req_notify_cq(cq, 0)) {
  //   rdma_log(RDMA_ERROR, "RDMAConnEvent::get_event_locked, require
  //   notifcation on rcq failed"); exit(-1);
  // }
...
//     rdma_log(RDMA_ERROR, "RDMAConn::post_send_and_poll_completion, MemRegion
//     incorrect"); exit(-1);
//   }
...
//     INIT_SGE(&send_sges_[posted_sr_tail_], (uint8_t*)local_mr.addr() +
//     local_offset, sz, local_mr.lkey()); INIT_SR(&send_wrs_[posted_sr_tail_],
//     &send_sges_[posted_sr_tail_], opcode, (uint8_t*)remote_mr.addr() +
//     remote_tail, remote_mr.rkey(), 1, sz); if (ibv_post_send(qp_,
//     &send_wrs_[posted_sr_tail_], &bad_wr) != 0) {
...
//     INIT_SGE(&send_sges_[posted_sr_tail_], (uint8_t*)local_mr.addr() +
//     local_offset, r_len, local_mr.lkey());
//     INIT_SR(&send_wrs_[posted_sr_tail_], &send_sges_[posted_sr_tail_],
//     opcode, (uint8_t*)remote_mr.addr() + remote_tail, remote_mr.rkey(), 1,
//     r_len); size_t second_sr_id = (posted_sr_tail_ + 1) %
//     DEFAULT_MAX_POST_SEND; INIT_SGE(&send_sges_[second_sr_id],
//     (uint8_t*)local_mr.addr() + local_offset + r_len, sz - r_len,
//     local_mr.lkey()); INIT_SR(&send_wrs_[second_sr_id],
//     &send_sges_[second_sr_id], opcode, remote_mr.addr(), remote_mr.rkey(), 1,
//     sz - r_len); send_wrs_[posted_sr_tail_].next = &send_wrs_[second_sr_id];
