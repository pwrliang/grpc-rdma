  ...
  public:
    RDMAServer() {
      ...
      if (setsockopt(sockfd_, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt))) {
          gpr_log(GPR_ERROR, "RDMAServer::RDMAServer, error on setsockopt (SO_REUSEADDR)");
          exit(-1);
      }
      opt = 1;
      if (setsockopt(sockfd_, SOL_SOCKET, SO_REUSEPORT, &opt, sizeof(opt))) {
          gpr_log(GPR_ERROR, "RDMAServer::RDMAServer, error on setsockopt (SO_REUSEPORT)");
          exit(-1);
      }

      pool_ = thpool_init(MAX_THREAD_NUM);
      if (!pool_) {
        gpr_log(GPR_ERROR, "RDMAServer::RDMAServer, error on thpool_init");
        exit(-1);
      }
    }
...
static void* thread_handler_bp(void* args) {
  ...
  while (true) {
    ...
    int read_size = rdmasr->recv(&msg, check_size);
...
static void* thread_handler_event(void* args) {
  ...
  rdmasr->connect(sockfd, reinterpret_cast<void*>(&sockfd));
  ...
  while (true) {
    ...
    for (size_t i = 0; i < r; i++) {
      ...
      if (fd == nullptr){
        printf("rdma_check_incoming failed\n");
        exit(-1);
      }
      readable_flag = true;
    }
    ...
    size_t mlen = rdmasr->get_unread_data_size();
    if (mlen >= DEFAULT_RINGBUF_SZ) {
      printf("wrong len: %zu\n", mlen);
    }
    struct msghdr msg;
    struct iovec iov;
...
int main(int argc, char *argv[]) {
  ...
  server.attach(thread_handler_event);
