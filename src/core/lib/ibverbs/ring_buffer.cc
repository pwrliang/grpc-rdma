...
retry:
  ...
  if (size > capacity_ - reserved_space) {
    LOG(ERROR) << "retry size " << size << " head " << head << " remain "
               << remain;
    goto retry;
  }
...
uint64_t RingBufferPollable::GetFreeSize(uint64_t head, uint64_t tail) const {
  ...
  uint64_t occupied = (tail + capacity_ - head) & capacity_mask_;
